    ...
    <>
      ...
      <section className="relative flex h-80 items-end overflow-hidden bg-ew-dark md:h-96">
        <div
          ...
          data-hygraph-field-api-id="coverImage"
        >
          {page.coverImage?.url ? (
            <Image src={page.coverImage.url} alt={page.title} fill className="object-cover" priority />
          ) : (
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-10 sm:px-6 lg:px-8">
          ...
          {page.airport && (
            <p className="mt-2 text-lg text-white/80">
              <span data-hygraph-entry-id={page.airport.id} data-hygraph-field-api-id="city">
                {page.airport.city}
              </span>
              , {page.airport.country} &middot; {page.airport.iataCode}
            </p>
          )}
